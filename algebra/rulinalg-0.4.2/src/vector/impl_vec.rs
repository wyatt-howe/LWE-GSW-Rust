...
impl<T> Vector<T> {
    ...
    /// use rulinalg::vector::Vector;
    ///
    /// let vec = Vector::new_bounded(vec![1.0,2.0,3.0,4.0], 256.0);
    /// ```
    pub fn new_bounded<U: Into<Vec<T>>>(data: U, q: T) -> Vector<T> {
        let our_data = data.into();
        let size = our_data.len();
    ...
    /// # }
    /// ```
    pub fn from_fn<F>(size: usize, mut f: F, q: T) -> Vector<T>
    ...
    /// Returns the element bound of the Vector.
    pub fn bound(&self) -> &Option<T> {
        &self.bound
    }

    // /// Sets the element bound of the Vector.
    // pub fn set_bound(&self, q: T) -> &Vector<T> {
    //     let mut v = self.into_vec();
    //     v.bound = Option::Some(q);
    //     &self.clone()
    // }
...
impl<T: Copy + Zero + Mul<T, Output = T> + Add<T, Output = T>> Vector<T> {
    ...
    pub fn dot(&self, v: &Vector<T>) -> T {
        utils::dot(&self.data, &v.data, &self.bound)
    }
}
